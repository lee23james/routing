...
def math_equal(
    ...
    try:  # 1. numerical equal
        ...
        _DEG_SUFFIX = re.compile(
            r"""^\s*
                ([+-]?(?:\d+(?:\.\d*)?|\.\d+))   # number
                \s*
                (?:°|deg(?:rees)?|\\degree|\^\s*\\circ|\^\s*\{\s*\\circ\s*\})
                \s*$
            """,
            re.IGNORECASE | re.VERBOSE,
        )

        def _strip_degree_token(x):
            """If x looks like '120°', '120^{\\circ}', or '120 deg', return '120'."""
            if isinstance(x, (int, float, bool)):
                return x
            s = str(x).strip()
            m = _DEG_SUFFIX.fullmatch(s)
            return m.group(1) if m else s
        prediction_norm = _strip_degree_token(prediction)
        reference_norm  = _strip_degree_token(reference)


        if is_digit(prediction_norm) and is_digit(reference_norm):
            prediction = parse_digits(prediction_norm)
            reference = parse_digits(reference_norm)
            # number questions
...
def _test_math_equal():
    ...
    print(math_equal('120°', '240/2'))
    print(math_equal('90^{\\circ}', '90'))
    print(math_equal('45 deg', '45'))
    gt = "x+2n+1"
    pred = "x+1"
