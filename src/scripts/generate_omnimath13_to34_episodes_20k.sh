#!/usr/bin/env bash
# Generate OmniMath 1<=diff<3 train-200 and 3<=diff<4 test-200 episodes with 20K decoding.
...
LOG_DIR="${LOG_DIR:-logs/trim_omnimath13_to34_generation}"
SUMMARY_JSON="${SUMMARY_JSON:-results/trim_omnimath13_omnimath34_search/context_saturation.json}"
...
generate_one omnimath_diff1_3_train_200 2>&1 | tee "$LOG_DIR/train_generation.log"
generate_one omnimath_diff3_4_test_200 2>&1 | tee "$LOG_DIR/test_generation.log"

train_path="$OUTPUT_DIR/omnimath_diff1_3_train_200_episodes.jsonl"
test_path="$OUTPUT_DIR/omnimath_diff3_4_test_200_episodes.jsonl"
if [ "$(wc -l < "$train_path")" -lt 200 ]; then
  echo "OmniMath 1-3 train episodes are incomplete: $train_path has $(wc -l < "$train_path") rows" >&2
  exit 1
fi
if [ "$(wc -l < "$test_path")" -lt 200 ]; then
  echo "OmniMath 3-4 test episodes are incomplete: $test_path has $(wc -l < "$test_path") rows" >&2
  exit 1
fi
...
"$PYTHON_BIN" scripts/summarize_episode_context.py \
  ...
  "$train_path" "$test_path" 2>&1 | tee "$LOG_DIR/context_saturation.log"

stop_generation_services
trap - EXIT

echo "[$(date '+%F %T')] generated $train_path and $test_path"
