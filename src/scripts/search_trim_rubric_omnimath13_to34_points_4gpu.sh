#!/usr/bin/env bash
# Search OmniMath 1<=diff<3 trained TRIM-Rubric PPO checkpoints on the local 4-GPU box.
...
RUBRIC_DIR="${RUBRIC_DIR:-data/rubrics/omnimath13_to34}"
...
LOG_DIR="logs/trim_rubric_omnimath13_to34_point_search"
...
run_one() {
  ...
  local tag="trim_rubric_omnimath13_to34_point_search_lam${lam_tag}_rub${rub_tag}_seed${SEED}"
...
echo "[$(date '+%F %T')] all OmniMath 1-3 TRIM-Rubric point-search training jobs finished"
