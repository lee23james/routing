...
CONDA_ENV="routing"
CONDA_SH="/home/honghudata/deepseek_VG/maker/anaconda3/etc/profile.d/conda.sh"
MODEL_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)/models"
...
[ "$SRM_RUNNING" -eq 0 ] && echo "  source $CONDA_SH && conda activate $CONDA_ENV"
[ "$SRM_RUNNING" -eq 0 ] && echo "  cd $VLLM_DIR && CUDA_VISIBLE_DEVICES=4 python server_vllm.py --model $MODEL_ROOT/qwen3-1.7b --port 4003 --tensor-parallel-size 1"
echo ""
[ "$LRM_RUNNING" -eq 0 ] && echo "  # LRM:"
[ "$LRM_RUNNING" -eq 0 ] && echo "  source $CONDA_SH && conda activate $CONDA_ENV"
