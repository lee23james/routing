...
GPU="${1:-1}"
EP_FILE="${2:-data/episodes/aime_train_episodes.jsonl}"
RUBRIC_WEIGHTS="${3:-data/rubrics/aime_train/rubric_weights.json}"
EPOCHS="${4:-200}"
LAM_RUBRIC="${LAM_RUBRIC:-0.3}"
DEVICE="${DEVICE:-cuda:0}"
OUT_ROOT="${OUT_ROOT:-checkpoints/trim_rubric_aime_official_sweep}"
LAM_VALUES="${LAM_VALUES:-3e-4 2.5e-4 2e-4 1.5e-4 1e-4 8e-5}"

if [ ! -f "$EP_FILE" ]; then
    echo "Missing episodes: $EP_FILE"
    echo "Generate it with: PYTHONPATH=src python -m data.generate_episodes --dataset aime_train --output_dir data/episodes"
    exit 1
fi
